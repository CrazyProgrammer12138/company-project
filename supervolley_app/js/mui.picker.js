...
(function($, window, document, undefined) {
	...
	Picker.prototype.setItems = function(items) {
         ...
         */
        if(isIos) {
            var num = self.list.angle / 20;
            var up = num, dw = num;
            var numArr = [];
            self.elementItems.forEach(function(item, index) {
                item.classList.remove("visible");
                item.classList.remove("highlight");
            });
            for(var i = 1; i <= 4; i++) {
            ...
            for(var i = 1; i <= 4; i++) {
                dw ++;
                if (dw > self.elementItems.length) break;
                numArr.push(dw);
            }
            setTimeout(function () {
                self.elementItems[num].classList.add("highlight")
                for (var i = 0; i < numArr.length; i++) {
                    self.elementItems[numArr[i]].classList.add("visible")
                }
            }, 50);
        }
    };
