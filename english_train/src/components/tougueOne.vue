<template>
  <div class="dayVoca">
    <div class="stduyFlow">请朗读并背诵常用情景对话，可通过“查看原文”学习发音，并于本周五通过微信录制音频发送给班主任老师
    </div>
    <div class="contentRead">
      <div class="titleRead">{{currentOral.title}}</div>
      <div class="timeRead">{{currentOral.beginTime | formatDate}}</div>
      <div class="textRead"></div>
      <div class="goKc" @click="goKc()">查看原文</div>
    </div>
...
export default {
  ...
  created () {
    ...
    $.ajax({
      type: 'get',
      url: _this.httpHl + '/camp/currentOral',
      data: {
