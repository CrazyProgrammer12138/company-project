<template>
  <div class="dayVoca">
    <ul class="lineKs">
      <li class="lineKsLi" data-keys="1" @click="gotoDetail($event, item.studyId)" v-for="(item, index) in pastListens" :key="index">
        <div class="meDiv">
...
export default {
  ...
  created () {
    ...
    $.ajax({
      type: 'get',
      url: _this.httpHl + '/camp/pastListens',
      data: {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (res.code === 200 || res.respCode === 200 || res.code === '200') {
            _this.audioUrl = res.data.audioUrl
