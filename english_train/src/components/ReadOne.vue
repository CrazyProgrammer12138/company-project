<template>
  <div class="dayVoca">
    ...
    <div class="contentRead">
      <div class="titleRead">{{currentRead.title}}</div>
      <div class="timeRead" v-cloak>{{currentRead.beginTime | formatDate}}</div>
...
export default {
  ...
  created () {
    ...
    $.ajax({
      type: 'get',
      url: _this.httpHl + '/camp/currentRead',
      data: {
      ...
      success: function (data) {
        ...
        if (res.code === 50000) {
          ...
          _this.currentRead = res.data
          $('.textRead').html(_this.currentRead.content)
