<template>
  <div class="dayVoca">
    <ul class="lineKs">
      <li class="lineKsLi" v-for="(items, index) in pastReads" :key="index" @click="gotoDetail($event, items.studyId)">
        <div class="meDiv">
          <img class="engImg" src="../views/activity/englishStudy/images/icos_1.png" alt="">
          <div class="rightEcl">
            <div class="meDical">
            </div>
            <div class="meDicalTwo">{{items.title}}</div>
            <div class="meDicalThree">{{items.beginTime}}</div>
          </div>
          <div class="wxk meUlRightRed" data-study="1" v-if="items.hasStudy == 0">未学习</div>
          <div class="wxk meUlRightColor" data-study="2" v-else>已学习</div>
          <!--<img :class="['redd',{isredd: items.hasStudy == 0}]" src="../../src/views/activity/englishStudy/images/redd.png" alt="">-->
        </div>
      </li>
    </ul>
...
export default {
  ...
  created () {
    ...
    $.ajax({
      type: 'get',
      url: _this.httpHl + '/camp/pastReads',
      data: {
